...
export default function DepartureInspectionPage() {
  ...
  const [unlockResult, setUnlockResult] = useState<UnlockVehicleResponse | null>(null)
  const [unlockError, setUnlockError] = useState<string | null>(null)

  const startInspectionMutation = useMutation({
    mutationFn: () => createDepartureInspection(reservationId),
    onSuccess: (response) => {
      setInspection(response.inspection)
      setBackendMissingFields(response.missing_fields)
      setMandatoryPhotoTypes(
        MANDATORY_PHOTO_ORDER.filter((photoType) => response.mandatory_photo_types.some((item) => isPhotoType(item) && item === photoType)),
      )
      setGlobalError(null)
      setGlobalSuccess('Etat des lieux initialise. Ajoutez les photos et renseignez les donnees de cloture.')
    },
    onError: (error) => {
  ...
  const uploadPhotoMutation = useMutation({
    mutationFn: async ({ photoType, file }: { photoType: PhotoType; file: File }) => {
      ...
      const photo = await uploadInspectionPhoto(inspection.id, {
        file,
    ...
    onError: (error, variables) => {
      setPhotoState((currentState) => ({
        ...currentState,
        [variables.photoType]: {
          ...currentState[variables.photoType],
  ...
  const completeInspectionMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setGlobalError(null)
      setGlobalSuccess('Etat des lieux termine avec succes. Vous pouvez maintenant deverrouiller le vehicule.')
  ...
  const canUnlockVehicle = inspection?.status === 'TERMINE' && !unlockMutation.isPending
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#2563EB]">Inspection de depart</p>
          <h1 className="mt-2 text-3xl font-semibold text-[#0F172A]">Etat des lieux de depart</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
            Reservation #{reservationId}. Prenez les photos obligatoires puis renseignez le kilometrage et l'energie.
          </p>
        </div>
      ...
      {!inspection ? (
        <Card>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-600">
              Lancez l'inspection pour obtenir les photos obligatoires et l'identifiant d'etat des lieux.
            </p>
        ...
        <div className="space-y-6">
                  ...
                  {requiredPhotoTypes.map((photoType) => {
                    ...
                    return (
                      <InspectionPhotoSlot
                        ...
                        isUploading={uploadPhotoMutation.isPending && uploadPhotoMutation.variables?.photoType === photoType}
          ...
          <Card header={<h2 className="text-lg font-semibold text-[#1F2937]">Cloture de l'etat des lieux</h2>}>
            <div className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <Input
                  ...
                  error={mileageInput.length > 0 && !isMileageValid ? 'Entrez un kilometrage entier superieur ou egal a 0.' : undefined}
                />
                ...
                <Input
                  ...
                  error={energyLevelInput.length > 0 && !isEnergyValid ? 'Entrez une valeur entre 0 et 100.' : undefined}
                />
              </div>
              ...
              {missingRequiredPhotos.length > 0 || localMissingFields.length > 0 ? (
                <Alert
                  ...
                  message={
                    <ul className="list-disc pl-5">
                      {missingRequiredPhotos.length > 0 ? <li>Photos manquantes: {missingRequiredPhotos.map((item) => PHOTO_LABELS[item]).join(', ')}</li> : null}
              ...
              <Button
                ...
                onClick={() => {
                  if (!inspection || mileageValue === null || energyLevelValue === null) {
                    return
                  }
          ...
          {inspection.status === 'TERMINE' ? (
            <Card>
              <div className="space-y-4">
                <p className="text-sm text-slate-700">Action suivante proposee: deverrouiller le vehicule.</p>
                {unlockResult ? (
                  <div className="space-y-3">
                    <Alert variant="success" title="Vehicule deverrouille" message={unlockResult.message} />
                    <div className="rounded-2xl border border-[#D1FAE5] bg-[#ECFDF5] p-4">
                      <div className="flex flex-wrap items-center gap-3">
                        <StatusBadge variant="success" label="Acces actif" />
                        <p className="text-sm text-[#065F46]">
                          Etat: <span className="font-semibold">{unlockResult.state}</span>
                        </p>
                      </div>
                      {unlockResult.timestamp ? (
                        <p className="mt-2 text-sm text-[#065F46]">
                          Horodatage: {new Date(unlockResult.timestamp).toLocaleString('fr-FR')}
                        </p>
                      ) : null}
                    </div>
                    <Link to={`/client/reservations/${reservationId}`}>
                ...
                {unlockError ? <Alert variant="danger" title="Deverrouillage impossible" message={unlockError} /> : null}
                <Button
                  disabled={!canUnlockVehicle}
                  onClick={() => {
                ...
                >
                  {unlockMutation.isPending ? (
                    <span className="flex items-center gap-2">
                      <LoadingSpinner size="sm" aria-label="Deverrouillage en cours" />
